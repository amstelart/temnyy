...
BasketPoolQuantity.prototype.updateQuantity = function()
{
	...
	var items_cart = items.getElementsByClassName('b-cart__item');
	//if (!!items && items.rows.length > 0)
	if (!!items && items_cart.length > 0)
	{
		//for (var i = 1; items.rows.length > i; i++)
		for (var i = 0; (items_cart.length)  > i; i++)
		{
			var itemId = items_cart[i].id;
...
function updateBasketTable(basketItemId, res)
{
	...
	var items_cart = table.getElementsByClassName('b-cart__item');
	//rows = table.rows;
	rows = items_cart;
	...
	if (basketItemId !== null && !!res.BASKET_DATA)
	{
		...
		newRow.setAttribute('class', 'b-cart__item');
		...
		for (i = 0; i < arColumns.length; i++)
		{
			switch (arColumns[i])
			{
				...
				case 'TYPE':
					break;
				case 'NAME':
			        oCellItem = document.createElement('div');
            		cellItemHTML = '';
					oCellItem.setAttribute('class', 'b-cart__item-product b-cart__cell b-cart__cell--product');
					oCellItem.setAttribute('id', 'basket_line_' + arItem.ID);
                    ...
                    {
                        cellItemHTML = '<a href="' + arItem.DETAIL_PAGE_URL + '" class="b-cart__item-image">\
                        	<img src="' + imageURL + '" alt="Джемпер EMPORIO ARMANI серый XXL">\
                        </a>\
                        <div class="b-cart__item-content">\
							<div class="b-cart__item-title">\
								<a href="' + arItem['DETAIL_PAGE_URL'] + '" class="b-cart__item-link">' + arItem['NAME'] + '</a>\
							</div>';
                        if (arItem.SKU_DATA)
                        {
                            for (propId in arItem.SKU_DATA)
                            {
                                if (arItem.SKU_DATA.hasOwnProperty(propId))
                                {
                                    ...
                                    if (bIsImageProperty)
                                    {
                                        ...
                                        cellItemHTML += '<ul id="prop_' + arProp['CODE'] + '_' + arItem['ID'] + '" style="margin-left:0%;" class="sku_prop_list">';
                                        ...
                                        for (valueId in arProp['VALUES'])
                                        {
                                            ...
                                            cellItemHTML += '<li class="sku_prop' + selected + '"\
                    ...
                    {
                        cellItemHTML = '<div class="bx_ordercart_photo_container">\
                    ...
                    {
                        cellItemHTML += '<div class="bx_ordercart_brand">\
                    ...
                    oCellItem = newRow.appendChild(oCellItem);
					break;
				case 'QUANTITY':
                    oCellQuantity = document.createElement('div');
                    oCellQuantityHTML = '';
					...
					oCellQuantity.setAttribute('class', 'b-cart__cell b-cart__cell--count b-cart__item-count');
					oCellQuantityHTML += '<div class="b-cart__item-mobile-title">' + getColumnName(res, arColumns[i]) + ':</div>\
											<div class="b-cart__item-content">\
                        						<div class="count-up">';

                    if (ratio != 0 && ratio != '' ) {
                        oCellQuantityHTML += '<a href="javascript:void(0);" class="count-up__btn minus small-basket-count_up" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'down\', ' + bUseFloatQuantity + ');">-</a>';
                    }
					oCellQuantityHTML += '<input type="text" size="3" id="QUANTITY_INPUT_' + arItem['ID'] + '"\
										name="QUANTITY_INPUT_' + arItem['ID'] + '" class="count-up__input"\
					...
					>';
					if (ratio != 0 && ratio != '' ) {
						oCellQuantityHTML += '<a href="javascript:void(0);" class="count-up__btn plus small-basket-count_up" onclick="setQuantity(' + arItem['ID'] + ', ' + ratio + ', \'up\', ' + bUseFloatQuantity + ');">+</a>';
					}

					// 	oCellQuantityHTML += '<td style="text-align: left">' + arItem['MEASURE_TEXT'] + '</td>';

					oCellQuantityHTML += '<input type="hidden" id="QUANTITY_' + arItem['ID'] + '" name="QUANTITY_' + arItem['ID'] + '" value="' + arItem['QUANTITY'] + '" /></div></div>';
                    ...
                    oCellQuantity = newRow.appendChild(oCellQuantity);
                    break;
				case 'PRICE':
                    oCellPrice = document.createElement('div');
                    fullPrice = (arItem['FULL_PRICE_FORMATED'] != arItem['PRICE_FORMATED']) ? arItem['FULL_PRICE_FORMATED'] : '';

					oCellPrice.setAttribute('class', 'b-cart__cell b-cart__cell--price b-cart__item-price');
                    oCellPrice.innerHTML += '<div class="b-cart__item-mobile-title">' + getColumnName(res, arColumns[i]) + ':</div>';
                    ...
                    oCellPrice = newRow.appendChild(oCellPrice);
                    break;
				case 'DISCOUNT':
                    oCellDiscount = document.createElement('div');
					oCellDiscount.setAttribute('class', 'b-cart__cell b-cart__cell--discount b-cart__item-discount');
					oCellDiscount.innerHTML = '<div class="b-cart__item-mobile-title">' + getColumnName(res, arColumns[i]) + ':</div>';
					oCellDiscount.innerHTML += '<div id="discount_value_' + arItem['ID'] + '">' + arItem['DISCOUNT_PRICE_PERCENT_FORMATED'] + '</div>';
                    oCellDiscount = newRow.appendChild(oCellDiscount);
                    break;
				case 'WEIGHT':
                    oCellWeight = document.createElement('div');
                    ...
                    oCellWeight = newRow.appendChild(oCellWeight);
                    break;
				default:
                    oCellCustom = document.createElement('div');
					customColumnVal = '';

					oCellCustom.setAttribute('class', 'b-cart__cell b-cart__cell--total b-cart__item-price b-cart__item-price--total');
					oCellCustom.innerHTML = '<div class="b-cart__item-mobile-title">' + getColumnName(res, arColumns[i]) + ':</div>';
                    ...
                    oCellCustom = newRow.appendChild(oCellCustom);
					break;
			}
		}
		if (bShowDeleteColumn || bShowDelayColumn)
		{
            oCellControl = document.createElement('div');
			//var oCellControl = newRow.insertCell(-1);
				oCellControl.setAttribute('class', 'b-cart__cell b-cart__cell--funcs b-cart__item-btns');

			if (bShowDeleteColumn)
				oCellControl.innerHTML = '<div class="b-cart__item-btn-wrap">\
					<a href="' + basketJSParams['DELETE_URL'].replace('#ID#', arItem['ID']) +'">' + basketJSParams['SALE_DELETE'] + '</a>\
       			 </div>';

			if (bShowDelayColumn)
				oCellControl.innerHTML += '<div class="b-cart__item-btn-wrap">\
				<a href="' + basketJSParams['DELAY_URL'].replace('#ID#', arItem['ID']) + '">' + basketJSParams['SALE_DELAY'] + '</a>\
				</div>';

            oCellControl = newRow.appendChild(oCellControl);
		}

		// set sku props click handler
		var sku_props = BX.findChildren(BX('basket_line_' + newBasketItemId), {tagName: 'li', className: 'sku_prop'}, true);

        if (!!sku_props && sku_props.length > 0)
		{
			for (i = 0; sku_props.length > i; i++)
			{
				console.log(sku_props[i]);
...
function couponCreate(couponBlock, oneCoupon)
{
	...
	var couponClassInput = couponClass + ' input-text';
	var couponClassSpan = couponClass + ' b-cart__coupon-item-remove';
	if (couponClass == 'bad') {
		var className = 'bx_ordercart_coupon b-cart__coupon-item ' + ' b-cart__coupon-item__' + couponClass + ' input-block--error';
	}
	else{
		var className = 'bx_ordercart_coupon b-cart__coupon-item ' + ' b-cart__coupon-item__' + couponClass;
	}
		...
		'div',
		{
			...
			children: [
				...
				BX.create(
					...
					{
						props: {
							className: 'bx_ordercart_coupon_notes hidden'
						},
			...
			if (!!couponsCollection)
			{
				...
				for (i = 0; i < res.COUPON_LIST.length; i++)
				{
					...
					if (couponFound)
					{
						...
						BX.adjust(couponsCollection[key], {props: {className: couponClassInput}});
						BX.adjust(couponsCollection[key].nextSibling, {props: {className: couponClassSpan}});
...
function recalcBasketAjax(params)
{
	...
	var property_values = {},
		...
		i;
	var items_cart = items.getElementsByClassName('b-cart__item');
	var delayeditems_cart = !!delayedItems ? delayedItems.getElementsByClassName('b-cart__item') : null;
	...
	if (!!items && items_cart.length > 0)
	{
		for (i = 0; items_cart.length > i; i++)
			postData['QUANTITY_' + items_cart[i].id] = BX('QUANTITY_' + items_cart[i].id).value;
	}

	//if (!!delayedItems && delayedItems.rows.length > 0)
	if (!!delayedItems && delayeditems_cart.length > 0)
	{
		for (i = 0; delayeditems_cart.length > i; i++)
			postData['DELAY_' + delayeditems_cart[i].id] = 'Y';
	}
